...
def calculate_incuba_interval_energy(
    ...
    if not start_candidates:
        raise IncubaIntradayInputError(
            "Incuba interval energy requires a power sample at or before the interval start."
        )
    start_sample = start_candidates[-1]
    end_sample = samples[-1]
    if start - start_sample[0] > MAX_BOUNDARY_AGE:
        raise IncubaIntradayInputError("The Incuba sample at the interval start is too old.")
    if end - end_sample[0] > MAX_BOUNDARY_AGE:
        raise IncubaIntradayInputError("The Incuba sample at the interval end is too old.")
...
def _latest_supported_origin(
    ...
    try:
        ...
        raise IncubaIntradayInputError(
            f"Could not retrieve the latest Incuba production timestamp: {exc}"
        ) from exc
    if latest is None:
        raise IncubaIntradayInputError("No Incuba production measurement is available.")

    observed_at = pd.Timestamp(getattr(latest, "timestamp_utc", None))
    if observed_at.tzinfo is None:
        raise IncubaIntradayInputError(
            "The latest Incuba production timestamp has no timezone."
        )
    supported_origin = min(
        wall_origin,
        observed_at.tz_convert(INCUBA_TIMEZONE).floor("15min"),
    )
    if wall_origin - supported_origin > pd.Timedelta(minutes=15):
        raise IncubaIntradayInputError(
            "The latest Incuba production measurement is too old."
        )
